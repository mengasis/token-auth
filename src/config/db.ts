...
export const initializeDB = async () => {
  try {
    ...
    await db.read();
    console.log("\x1b[32m%s\x1b[0m", "Database file initialized and ready for use."); // Verde
  } catch (error) {
    console.error("\x1b[31m%s\x1b[0m", "Failed to initialize the application:", error); // Rojo
    process.exit(1);
  }
}
